...
player-start validation, and strict lossless JSON syntax APIs. It does not
provide general project understanding, project validity, editor compatibility,
or modification.
...
`tilesets`, `map`, `system`, and `validate` may resolve root or ancestor
symlinks and does not prove root identity. The `inspect-json` command explicitly
opens the provided path and makes no project-containment claim.
