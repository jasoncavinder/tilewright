...
| Map file family | **Partial, including child creation and reparenting** | `MapInfos.json` plus three-digit `MapNNN.json` is documented and observed for 196 maps through ID 189. Controlled lifecycle actions distinguish ID from compact order. Child ID 4 uses `parentId`; reparenting changes parent/order while preserving IDs/files, and expansion values persist independently. | Deeper hierarchy, parent deletion, multiple-hole selection, IDs above 999, orphan files, cycles, and enforcement. |
