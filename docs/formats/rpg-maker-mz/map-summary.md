...
The accepted next experimental typed slice will accept an existing
`ProjectSnapshot` and a catalog-scoped `MapId`, then expose only:
...
The operation will require a structurally coherent map catalog and a matching
...
diagnostics. It will refuse missing, duplicate, wrong-kind, or unsupported
