...
| `tilewright` CLI executable | Experimental | Provides help/version output and human- or JSON-formatted access to the core library's experimental explicit-root candidate discovery and capability-relative project inventory. Initial `open_ambient_dir` acquisition may resolve root or ancestor symlinks and does not prove root identity. It does not load, validate, or modify projects. |
