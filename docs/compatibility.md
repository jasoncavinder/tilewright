...
| `tilewright` CLI executable | Experimental | Provides help/version output and human- or JSON-formatted access to the core library's experimental explicit-root candidate discovery, capability-relative project inventory, and strict lossless JSON syntax inspection. Initial `open_ambient_dir` acquisition may resolve root or ancestor symlinks and does not prove root identity. The `inspect-json` command explicitly opens the provided path and makes no project-containment claim. It does not load, validate, or modify projects. |
