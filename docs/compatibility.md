...
| --- | --- | --- |
| Core `tilewright` package | Experimental | Exposes its package version, candidate discovery, capability-relative read-only project inventory, and immutable strict lossless JSON syntax representation. |
| `tilewright` CLI executable | Experimental | Provides help/version output and human- or JSON-formatted access to the core library's experimental explicit-root candidate discovery and capability-relative project inventory. It does not load, validate, or modify projects. |
...
| Project-file parsing | Not implemented | The generic syntax representation exists experimentally, but no project loader, document identity model, partial-snapshot behavior, or format models exist. |
